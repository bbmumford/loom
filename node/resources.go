...
func detectMemory() int64 {
	...
	if memLimit := os.Getenv("MEMORY_LIMIT_MB"); memLimit != "" {
		if mb, err := strconv.ParseInt(memLimit, 10, 64); err == nil {
			dbgResources.Printf("Using MEMORY_LIMIT_MB from environment: %dMB", mb)
			return mb
		}
	}
	...
	dbgResources.Printf("Warning: Using runtime heap size as memory estimate: %dMB", heapMB)
	return heapMB
}
...
func detectCPU() int {
	...
	if cpuCores := os.Getenv("CPU_CORES"); cpuCores != "" {
		if cores, err := strconv.Atoi(cpuCores); err == nil && cores > 0 {
			dbgResources.Printf("Using CPU_CORES from environment: %d", cores)
			return cores
		}
	}
...
func calculateMaxRoles(memoryMB int64, cpuCores int) int {
	...
	if maxRolesEnv := os.Getenv("MAX_ROLES"); maxRolesEnv != "" {
		if maxRoles, err := strconv.Atoi(maxRolesEnv); err == nil && maxRoles > 0 {
			dbgResources.Printf("Using MAX_ROLES from environment: %d", maxRoles)
			return maxRoles
		}
	}
...
func (r *SystemResources) LogResources() {
	dbgResources.Printf("System Resources Detected:")
	dbgResources.Printf("  Memory:      %dMB", r.MemoryMB)
	dbgResources.Printf("  CPU Cores:   %d", r.CPUCores)
	dbgResources.Printf("  Max Roles:   %d concurrent", r.MaxRoles)
	dbgResources.Printf("  Capacity:    %.1f%% available", r.Capacity*100)
}
...
// init function to validate that gopsutil is available
func init() {
	// Test if gopsutil is working
	if _, err := mem.VirtualMemory(); err != nil {
		log.Printf("Warning: gopsutil memory detection unavailable, falling back to env vars/runtime: %v", err)
	}
	if _, err := cpu.Counts(true); err != nil {
		log.Printf("Warning: gopsutil CPU detection unavailable, falling back to env vars/runtime: %v", err)
	}
}
